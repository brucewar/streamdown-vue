...
describe("MermaidFullscreenButton", () => {
  ...
  it("should lock body scroll when fullscreen opens", async () => {
    const { container } = renderWithContext({ chart: "graph TD; A-->B" });
  ...
  it("should stop keydown propagation on content area", async () => {
    ...
    const contentArea = document.querySelector('[role="presentation"]');
    // biome-ignore lint/style/noNonNullAssertion: test assertion
    await fireEvent.keyDown(contentArea!, { key: "Escape" });
  ...
  it("should render accessible fullscreen dialog", async () => {
    const { container } = renderWithContext({ chart: "graph TD; A-->B" });
    ...
    await waitFor(() => {
      ...
      expect(dialog?.getAttribute("aria-label")).toBe("View fullscreen");
    });
  });
