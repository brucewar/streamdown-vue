...
describe("ImageComponent cached image handling", () => {
  it("should detect cached image on mount via img.complete and naturalWidth", async () => {
  ...
  it("should handle image error on cached load (complete but naturalWidth=0)", async () => {
    const { container } = render(
      <StreamdownContext.Provider
        value={{
          shikiTheme: ["github-light", "github-dark"],
...
describe("TableDownloadDropdown error on save throw (line 78)", () => {
  it("should call onError when save throws in dropdown", async () => {
    ...
    await fireEvent.click(csvBtn as HTMLButtonElement);

    expect(onError).toHaveBeenCalledWith(expect.any(Error));
  });
});

describe("components.tsx code extraction from VNodeChild children (line 729)", () => {
  it("should extract code from VNodeChild children with string content", async () => {
