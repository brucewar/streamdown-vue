...
describe("PanZoom", () => {
  ...
  it("should hide controls when showControls is false", () => {
    ...
    expect(wrapper.find('button[title="Zoom in"]').exists()).toBe(false);
  });
  ...
  it("should reset zoom and pan when reset button is clicked", async () => {
    ...
    await wrapper.find('button[title="Zoom in"]').trigger("click");
    await wrapper.find('button[title="Reset zoom and pan"]').trigger("click");
  ...
  it("should respect minZoom limit", () => {
    ...
    expect(wrapper.find('button[title="Zoom out"]').attributes("disabled")).toBeDefined();
  });

  it("should respect maxZoom limit", () => {
    const wrapper = mountPanZoom({ initialZoom: 3, maxZoom: 3, zoomStep: 0.1 });

    expect(wrapper.find('button[title="Zoom in"]').attributes("disabled")).toBeDefined();
  });
