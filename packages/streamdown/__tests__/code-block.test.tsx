...
describe("CodeBlock with multiple languages", () => {
  ...
  it("should handle heavy streaming appropriately", async () => {
    ...
    const StreamingCodeBlock = defineComponent({
      setup() {
        const streamedCode = ref("");
        setStreamedCodeExternal = (code: string) => {
          streamedCode.value = code;
        };

        return () =>
          h("div", [h(CodeBlock, { code: streamedCode.value, language: "html" })]);
      },
    });
