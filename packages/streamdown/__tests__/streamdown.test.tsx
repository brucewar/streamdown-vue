import { render } from "./helpers/testing-library-vue";
...
describe("Streamdown Component", () => {
  ...
  it("should handle non-string children", () => {
    const content = <div>Vue Element</div>;
    const { container } = render(<Streamdown>{content as any}</Streamdown>);
    const wrapper = container.firstElementChild;
    expect(wrapper).toBeTruthy();
    expect(wrapper?.textContent).toBe("Vue Element");
  });
  ...
  it("should update when children change", async () => {
    ...
    await rerender(<Streamdown className="class2">Content</Streamdown>);
    await rerender(<Streamdown className="class2">Different Content</Streamdown>);
  ...
  it("should handle number children", () => {
    ...
    const wrapper = container.firstElementChild;
    expect(wrapper).toBeTruthy();
    expect(wrapper?.textContent).toBe("123");
  });
...
describe("animation callbacks", () => {
  it("fires onAnimationStart when isAnimating transitions false → true", async () => {
  ...
  it("fires onAnimationEnd when isAnimating transitions true → false", async () => {
  ...
  it("does not fire callbacks when isAnimating is unchanged across re-renders", async () => {
    const onAnimationStart = vi.fn();
    const onAnimationEnd = vi.fn();
    const { rerender } = render(
      <Streamdown
  ...
  it("fires callbacks correctly across multiple animation cycles", async () => {
    const onAnimationStart = vi.fn();
    const onAnimationEnd = vi.fn();
    const { rerender } = render(
      <Streamdown
  ...
  it("does not throw when callbacks are not provided and isAnimating toggles", async () => {
    const { rerender } = render(
      <Streamdown isAnimating={false}>content</Streamdown>
    );
    await expect(
      (async () => {
        await rerender(<Streamdown isAnimating={true}>content</Streamdown>);
        await rerender(<Streamdown isAnimating={false}>content</Streamdown>);
      })()
    ).resolves.toBeUndefined();
  });

  it('suppresses callbacks in mode="static"', async () => {
    const onAnimationStart = vi.fn();
    const onAnimationEnd = vi.fn();
    const { rerender } = render(
      <Streamdown
